...
struct Frame
{
	...
	Frame(const uint64_t _id, const uint64_t _formula) :
        		formulas(), toProcess(), eventualities(), id(_id), choosenFormula(MAX_FORMULA), choice(false), chain(nullptr)
	{
	...
	Frame(const uint64_t _id, const Frame& _frame) : 
		formulas(_frame.formulas), toProcess(_frame.toProcess), eventualities(_frame.eventualities),
		id(_id), choosenFormula(MAX_FORMULA), choice(false), chain(_frame.chain) { }
	...
	Frame(const uint64_t _id, const std::unordered_map<uint64_t, uint64_t> _eventualities, const Frame* chainPtr) :
		formulas(), toProcess(), eventualities(_eventualities), id(_id),
		choosenFormula(MAX_FORMULA), choice(false), chain(chainPtr)
	{
		toProcess.set();
	}
};
...
static std::pair<std::vector<FormulaPtr>, uint64_t> initialize(const FormulaPtr f)
{
    ...
    for (auto& _f : formulas)
    {
        ...
        if (left)
        {
            lhs = std::find_if(formulas.begin(), formulas.end(), [&] (FormulaPtr a) { return a == left; }) - formulas.begin();
            assert(std::find_if(formulas.begin(), formulas.end(), [&] (FormulaPtr a) { return a == left; }) != formulas.end());
        }
        if (right)
        {
            rhs = std::find_if(formulas.begin(), formulas.end(), [&] (FormulaPtr a) { return a == right; }) - formulas.begin();
            assert(std::find_if(formulas.begin(), formulas.end(), [&] (FormulaPtr a) { return a == right; }) != formulas.end());
        }
                                                                         ...
                                                                         const std::stack<Frame>& stack, const Frame& loopTo)
{
	...
	for (const auto& frame : Container(stack))
	{
		...
		{
			if (frame.formulas[j] &&
                (atom_set.find(j) != atom_set.end() ||
                 (not_bitset[j] && atom_set.find(lhs_set[j]) != atom_set.end())
                )
               )
				set.insert(fs[j]);
		}
...
loop:
   ...
   else
   {
   	...
   	return __parallel_impl(formulas, start, params.generateModel, params.seed);
   }
   */
}
