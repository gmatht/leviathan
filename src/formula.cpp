...
#define MAKE_UNARY(_Type, _type) _Type##Ptr make_##_type(const FormulaPtr& f) \
{ \
	return std::make_shared<_Type>(f); \
}

#define MAKE_BINARY(_Type, _type) _Type##Ptr make_##_type(const FormulaPtr& f1, const FormulaPtr& f2) \
{ \
	return std::make_shared<_Type>(f1, f2); \
}

#define ACCEPT_VISITOR(_Type) void _Type::accept(Visitor& v) const \
{ \
	v.visit(this); \
}
...
#define UNARY_EQ(_Type) if (isa<_Type>(a)) \
	return fast_cast<_Type>(a)->formula() == fast_cast<_Type>(b)->formula();

#define BINARY_EQ(_Type) if (isa<_Type>(a)) \
	return (fast_cast<_Type>(a)->left() == fast_cast<_Type>(b)->left()) &&\
	             (fast_cast<_Type>(a)->right() == fast_cast<_Type>(b)->right());
