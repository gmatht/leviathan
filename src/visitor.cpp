...
#define UNARY_VISIT(_Type, _Symbol) void PrettyPrinter::visit(const _Type* u) \
{ \
	std::cout << _Symbol << "( "; \
...
#define BINARY_VISIT(_Type, _Symbol) void PrettyPrinter::visit(const _Type* b) \
{ \
	std::cout << "("; \
	b->left()->accept(*this); \
	std::cout << ") " << _Symbol <<" ("; \
...
void Simplifier::visit(const Tomorrow* t)
{
	...
	else if (isa<False>(result))
	{
		...
		rulesApplied = true;
	else if (isa<Conjunction>(result) && isa<Always>(fast_cast<Conjunction>(result)->right()) &&
	              isa<Eventually>(fast_cast<Always>(fast_cast<Conjunction>(result)->right())->formula()))
	{
		result = make_conjunction(make_tomorrow(fast_cast<Conjunction>(result)->left()),
					        fast_cast<Conjunction>(result)->right());
		rulesApplied = true;
	}
	else if (isa<Conjunction>(result) && isa<Always>(fast_cast<Conjunction>(result)->left()) &&
	              isa<Eventually>(fast_cast<Always>(fast_cast<Conjunction>(result)->left())->formula()))
	{
		result = make_conjunction(fast_cast<Conjunction>(result)->left(),
					        make_tomorrow(fast_cast<Conjunction>(result)->right()));
		rulesApplied = true;
	}
	else if (isa<Disjunction>(result) && isa<Always>(fast_cast<Disjunction>(result)->right()) &&
	              isa<Eventually>(fast_cast<Always>(fast_cast<Disjunction>(result)->right())->formula()))
	{
		result = make_disjunction(make_tomorrow(fast_cast<Disjunction>(result)->left()),
					       fast_cast<Disjunction>(result)->right());
		rulesApplied = true;
	}
	else if (isa<Disjunction>(result) && isa<Always>(fast_cast<Disjunction>(result)->left()) &&
	              isa<Eventually>(fast_cast<Always>(fast_cast<Disjunction>(result)->left())->formula()))
	{
...
void Simplifier::visit(const Conjunction* c)
{
	...
	else if ((isa<Negation>(left) && right == fast_cast<Negation>(left)->formula()) ||
	              (isa<Negation>(right) && left == fast_cast<Negation>(right)->formula()))
	{
		result = make_false();
		rulesApplied = true;
	}
...
void Simplifier::visit(const Disjunction* d)
{
	...
	else if (isa<Always>(left) && isa<Eventually>(fast_cast<Always>(left)->formula()) &&
	              isa<Always>(right) && isa<Eventually>(fast_cast<Always>(right)->formula()))
	{
