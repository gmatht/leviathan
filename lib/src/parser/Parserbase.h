...
template <>
struct TagOf<LTL::FormulaPtr> {
  static Tag__ const tag = Tag__::FORMULA;
};

template <>
struct TagOf<std::string> {
  static Tag__ const tag = Tag__::ATOM;
};

template <>
struct TypeOf<Tag__::FORMULA> {
  typedef LTL::FormulaPtr type;
};

template <>
struct TypeOf<Tag__::ATOM> {
...
inline Semantic<tg_>::Semantic()
  : Base(tg_), d_data(typename TypeOf<tg_>::type())
{
}
