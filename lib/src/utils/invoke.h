...
auto invoke(F &&f, Args &&... args)
   ...
   */
  template <typename R, typename C, typename Obj, typename... FArgs,
            typename... Args>
  ...
  auto invoke(T(C::*m), Obj &&obj) declreturn(std::forward<Obj>(obj).*m)

  /*
   * Pointers to members
   */
  template <typename T, typename C, typename Obj>
  auto invoke(T(C::*m), Obj *obj) declreturn(obj->*m)
...
public:
  using type =
    typename std::conditional<std::is_class<U>::value, U, holder<U>>::type;
};
 ...
 */
template <typename T>
auto invokable(T &&t) declreturn(invokable_t<T>(std::forward<T>(t)))
  ...
  auto apply_impl(F &&f, Tuple &&tuple, std14::index_sequence<Idx...>)
    declreturn(invoke(std::forward<F>(f),
      ...
      auto apply(F &&f, Tuple &&tuple) declreturn(apply_impl(
