 ...
 * trait class called has_member_save, which take a class and a function
 * signature.
 ...
 * and 'doesnt_have_member_save_tag' otherwise. This allows an easy tag
 * dispatch.
 *
 *
 ...
 *     template<typename T>
 *     bool func(T obj) {
 *        return save_if_possible(obj, has_member_save<T,
 * bool(std::string)>());
 *     }
 *
 *     template<typename T>
...
#define DECLARE_HAS_MEMBER_TRAIT(Member)                                      \
  ...
  struct doesnt_have_member_##Member##_tag : public std::false_type {         \
                                                                              ...
                                                                              \
  template <typename T, typename S = void, bool _ = std::is_class<T>::value>  \
  ...
  private:                                                                    \
    ...
    static std::false_type invoke(...);                                       \
                                                                              \
    using _return_type =                                                      \
      decltype(invoke(std::declval<T>(), std::declval<Args>()...));           \
    static const bool _has_member = _return_type::value;                      \
                                                                              \
  public:                                                                     \
    static const bool value =                                                 \
      _has_member && (std::is_void<R>::value ||                               \
                      std::is_convertible<_return_type, R>::value);           \
    using type =                                                              \
      typename std::conditional<value, has_member_##Member##_tag,             \
  ...
  using has_member_##Member =                                                 \
    typename has_member_##Member##_trait<T, F>::type;
