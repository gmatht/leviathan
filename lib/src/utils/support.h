...
#define code_unreachable() \
  ::utils::details::unreachable(__FILE__, __LINE__, __func__);
#endif

namespace utils {
namespace details {

[[noreturn]] inline void unreachable(const char *file, unsigned line,
                                     const char *func)
{
  fprintf(stderr,
          "Execution reached code_unreachable() at %s:%d in function %s", file,
          line, func);
  abort();
}
