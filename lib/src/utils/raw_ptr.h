...
public:
  ...
  ptr_base(ptr_base &&other) noexcept { std::swap(_ptr, other._ptr); }
  ...
  bool operator==(ptr_base other) noexcept { return _ptr == other._ptr; }
  bool operator!=(ptr_base other) noexcept { return _ptr != other._ptr; }
...
public:
  ...
  template <typename U, REQUIRES(std::is_convertible<U *, T *>())>
