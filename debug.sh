...
echo "`< debug__$D.out grep Total.frames | sed s/.*:.// | average.sh 2> /dev/null`" `cat debug__$D.err` " ($D) " 
done
...
do
for i in `seq 1 $N`
do JOB_NO=$i/$N@$D JOB2_NO=1/1@1 JOB3_NO=1/1@1 time bin/checker -v 4  -l '(G (a => X (b|c))) & (G (b => X a)) & (G (c => X a)) & a & X X X X X X X X X X ~a' # &
done > debug$D.out 2> debug$D.err 
wait
< debug$D.out grep Total.frames | sed s/.*:.// | average.sh 2> /dev/null
done
